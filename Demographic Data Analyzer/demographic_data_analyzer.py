...
def calculate_demographic_data(print_data=True):
    ...
    # value_counts() counts the total of each variable
    race_count = df.race.value_counts()

    # What is the average age of men?
    # with df.loc[df.sex == "Male"] we get all sample with male in the sex column
    # .age gives us the age column and with .mean() we get the overall mean of that
    male_age_mean = df.loc[df.sex == 'Male'].age.mean()
    average_age_men = round(male_age_mean, 1)
    ...
    higher_education_rich = round(
        (higher_education[higher_education.salary == '>50K'].salary.count() / higher_education.shape[0]) * 100, 1)
    lower_education_rich = round(
        (lower_education[lower_education.salary == '>50K'].salary.count() / lower_education.shape[0]) * 100, 1)
    ...
    rich_percentage = round(
        (num_min_workers[num_min_workers.salary == '>50K'].salary.count() / num_min_workers.shape[0]) * 100, 1)
